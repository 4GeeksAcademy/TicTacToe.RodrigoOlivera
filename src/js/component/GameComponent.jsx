...
const GameComponent = ({Players}) => {
    const initialState = [
        {
            row:"row 1",
            casillas: [{value:undefined,player:undefined},{value:undefined,player:undefined},{value:undefined,player:undefined}]
        },
        {
            row:"row 2",
            casillas: [{value:undefined,player:undefined},{value:undefined,player:undefined},{value:undefined,player:undefined}]
    ...
    useEffect(()=>{
        ...
        lugares.map((lugar,indexRow)=> {
            const RowInString = lugar.casillas[0].value + lugar.casillas[1].value + lugar.casillas[2].value
            if(RowInString  === "XXX") {
                setStyleWinner({
                    pointerEvents: "none",
                    opacity: "0.5"
            ...
            const valoresEnDiagonalDesdeDerecha = valorDiagonal1 + valorDiagonal2 + valorDiagonal3
            ...
            const valoresEnDiagonalDesdeIzquierda = valorDiagonal1Inverso + valorDiagonal2Inverso + valorDiagonal3Inverso
        ...
        <div className="container ">
            <div className="row">
                <div className="col-6 m-auto d-flex flex-column align-content-center">
                    <h1 className={`text-center text-${ganador !== ""? `success` : `white`}`} >{ganador !== ""? "el ganador es " + ganador + " :D" : 'Turn ' + turno} </h1>
                    <button className="m-auto" onClick={()=> final() }>Start Again</button>
                </div>
            </div>
                        ...
                        lugares.map((lugar,indexRow)=> {
                            return <div className="row" >
                                {lugar.casillas.map((casilla,indexCol)=> {
                                    ...
                                    return <div onClick={()=>RellenarCasilla(indexCol,indexRow)} role="button" className={"col-4 d-flex flex-row justify-content-center align-items-center"} style={styleCard}>
                                        <h1>{casilla.value }</h1>
                                    </div>
                                })}
                            </div>
